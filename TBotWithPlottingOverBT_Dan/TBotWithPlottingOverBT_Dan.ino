...
int incflag;
char character;
// Dan's Variables for auto-tune
float accu_motor = 0.0; // accumuated motor movement, used for auto-tune
float accu_limit = 2.0; // auto-tune limit
float gtrim_step = 0.02; // shift in gtrim in auto-tune
///////   Tuning ////////////////////////////////////////////

float gtrim = 13.5; // 16.0 for Titus, 3.5 for Maximus, 13.5 for SamBot  // Compensated for drift in forward or reverse direction.
...
float plotrange[2] = {gtrim-10, gtrim+10};
...
void gyroPIDCallBack() {
    ...
    vel(h,gyroyOutput); // output vxy

    // Autotune, By Dan P
    if (vxy<5 && abs(joyYf)<0.01 && abs(joyXf)<0.01){
      accu_motor += vxy;
    }
    if (accu_motor>accu_limit){
      accu_motor = 0.0;
      gtrim += gtrim_step;
    }
    else if (accu_motor<-accu_limit){
      accu_motor = 0.0;
      gtrim -= gtrim_step; 
    }
    // Check gtrip
    if (abs(gtrim-gtrim_ini)>2.0){
      gtrim = gtrim_ini; 
    }
    // End of Autotune
